...
const IconBurgerMenu: FC<{
  isOpen: boolean;
  onClick: () => void;
  color?: string;
}> = ({ isOpen, onClick, color }) => (
  <button
    onClick={onClick}
    aria-label="Menu"
    className={cn("w-7 h-7 relative focus:outline-none")}
  >
    <div className="absolute left-1/2 top-1/2 block w-7 -translate-x-1/2 -translate-y-1/2">
      <span
        aria-hidden="true"
        className={cn(
          "block absolute h-0.5 w-7 transform transition duration-500 ease-in-out",
          {
            "rotate-45": isOpen,
